...
> The backend is the `src/smart_search` Python CLI package, the Python 3.12 document-mining sidecar under `sidecar/`, and the `npm/` distribution layer. There is no frontend in this repository — the read-only Research Workspace visualizer is a bundled single-page asset served by `src/smart_search/assets/research_visualizer/server.py`, so no frontend spec layer exists.
...
| [Packaging Contract](./packaging-contract.md) | npm tarball / wheel / installed skill files sync, bundled-asset coverage, version sync | Active |
...
Repository structure is documented once, at the root [STRUCTURE.md](../../../STRUCTURE.md) — do not duplicate it here. Research-runtime concept definitions (Root Agent, Source Curator, SearchTask, EvidenceItem, ...) live in the project termbase `.terminology/termbase.json`.

## House Rules

1. Specs describe the code as it is, with real file pointers; update them in the same change as the behavior.
