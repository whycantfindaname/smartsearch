...
- The bundled delegated AnySearch adapter additionally recognizes `ANYSEARCH_API_KEY_FALLBACK` from the Smart Search config file and never reads AnySearch `.env` or `runtime.conf`; those local files are preserved during Skill refreshes.
