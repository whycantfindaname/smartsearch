...
`--timeout` defaults to `120` seconds and `--max-try` defaults to `5`. The
budget is limited to the safe logical replay cases listed in the catalog. Do not add an agent-side retry loop around this command. The catalog, rather than this example file, is the place to record a new error handling rule.
