...
from . import activity, research_runtime, service
...
def _write_skill_install_summary(result: dict[str, Any], lang: str) -> None:
    ...
    for companion in result.get("companions") or []:
        for item in companion.get("installed") or []:
            _write_stderr(f"  [OK] {item.get('label')} / {item.get('skill')} -> {item.get('path')}\n")
